...
# --- HỆ THỐNG XỬ LÝ ĐIỂM SỐ & SINH NHẬN XÉT OFFLINE TỰ NHIÊN ---
def get_mdd(val):
    val = str(val).strip().upper()
    if val in ["T", "HTT", "TỐT", "A", "A+"]: return "T"
    elif val in ["K", "KHÁ", "B", "B+"]: return "H_Kha"
    elif val in ["Đ", "HT", "ĐẠT", "H"]: return "H_TrungBinh"
    elif val in ["C", "CHT", "CHƯA ĐẠT", "D", "E"]: return "C"
    return "H_TrungBinh" 

def calc_overall_mdd(vals):
    valid_vals = [str(v).strip().upper() for v in vals if str(v).strip() != '']
    if not valid_vals: return "H_TrungBinh"
    mapped = [get_mdd(v) for v in valid_vals]
    if "C" in mapped: return "C"
    if all(v == "T" for v in mapped): return "T"
    if all(v in ["T", "H_Kha"] for v in mapped): return "H_Kha"
    return "H_TrungBinh"

def sinh_nhan_xet_pcnl_offline(mdd_nlc, mdd_nldt, mdd_pc):
    def cap_first(s):
        if not s: return ""
        s = s.strip()
    ...
    nlc_kho = {
        "T": ["tự học và giao tiếp cực kỳ xuất sắc", "tiếp thu nhạy bén các kỹ năng chung", "kỹ năng giải quyết vấn đề linh hoạt", "năng lực tự chủ và giao tiếp rất tốt", "ý thức tự học cao"],
        "H_Kha": ["ý thức tự học và giao tiếp khá", "hợp tác nhóm linh hoạt", "tự chủ trong giải quyết vấn đề khá tốt", "thực hiện khá tốt các năng lực chung"],
    ...
    nldt_kho = {
        "T": ["năng lực đặc thù vô cùng nổi trội", "tiếp thu rất tốt kiến thức các môn học", "phát huy xuất sắc năng lực chuyên biệt", "tính sáng tạo rất cao trong học tập"],
        "H_Kha": ["nắm vững kiến thức đặc thù các môn", "phát huy khá tốt năng lực bộ môn", "kết quả khá ở các môn học", "kỹ năng thực hành bộ môn khá vững"],
    ...
    pc_kho = {
        "T": ["luôn nỗ lực, trách nhiệm và hòa đồng", "biết quan tâm, chia sẻ và trung thực", "chăm chỉ, kỷ luật và ngoan ngoãn", "tinh thần yêu nước, nhân ái tuyệt vời"],
        "H_Kha": ["chăm chỉ và trung thực trong học tập", "có trách nhiệm với nhiệm vụ tập thể", "luôn tuân thủ nội quy lớp học", "ý thức rèn luyện khá tốt các phẩm chất"],
        "H_TrungBinh": ["biết yêu thương và tuân thủ nội quy", "chăm chỉ nhưng đôi khi thiếu tính tự giác", "biết vâng lời nhưng cần rèn kỷ luật cá nhân", "ý thức đạo đức tốt nhưng cần mạnh dạn hơn"],
    ...
    khuyen_nldt = {"T": ["Kết quả rèn luyện rất xứng đáng.", "Tuyệt vời!"], "H_Kha": ["Chỉ cần cố gắng thêm chút nữa sẽ rất xuất sắc.", "Tiếp tục nỗ lực nhé."], "H_TrungBinh": ["Hãy tập trung tối đa cho việc học.", "Cần chăm chỉ hơn."], "C": ["Cần nỗ lực gấp đôi để khắc phục.", "Phải cố gắng rất nhiều."]}
    khuyen_pc = {"T": ["Là tấm gương sáng cho cả lớp.", "Rất đáng tự hào."], "H_Kha": ["Sự rèn luyện rất đáng khen.", "Cần duy trì thái độ này."], "H_TrungBinh": ["Cần chấn chỉnh lại ý thức kỷ luật.", "Cố gắng khắc phục nhé."], "C": ["Yêu cầu nghiêm túc tuân thủ nội quy.", "Cần thay đổi thái độ học tập."]}
    
    nx_nlc, nx_nldt, nx_pc = "", "", ""
    
    if mdd_nlc: nx_nlc = f"{cap_first(random.choice(nlc_kho[mdd_nlc]))}. {random.choice(khuyen_nlc[mdd_nlc])}"
    if mdd_nldt: nx_nldt = f"{cap_first(random.choice(nldt_kho[mdd_nldt]))}. {random.choice(khuyen_nldt[mdd_nldt])}"
    if mdd_pc: nx_pc = f"{cap_first(random.choice(pc_kho[mdd_pc]))}. {random.choice(khuyen_pc[mdd_pc])}"
            
    return nx_nlc, nx_nldt, nx_pc
...
def phan_tich_file(file, thoi_diem):
    try:
        ...
        # QUÉT VÀ PHỤC HỒI TIÊU ĐỀ BỊ GỘP (MERGE CELLS) CHIỀU NGANG
        header_matrix = []
        for r in range(max(0, h_row - 4), s_row):
            row_vals = []
            last_val = ""
            for j in range(len(df.columns)):
                val = str(df.iloc[r, j]).strip()
        ...
        for j in range(n_col + 1, len(df.columns)):
            col_header_words = [header_matrix[r - max(0, h_row - 4)][j].lower() for r in range(max(0, h_row - 4), s_row)]
            header_area = " ".join(col_header_words)
            ...
            if is_my_term and not is_nx and check_col_has_data(df, j, s_row, "level"):
...
if f_hs:
    df_raw, n_col, d_col, m_col, c_col, s_row, pcnl_cols, valid_eval_cols = phan_tich_file(f_hs, thoi_diem)
    ...
    if df_raw is not None:
        ...
        if loai_nx == "PC-NL" and valid_eval_cols:
            for j in valid_eval_cols:
                h_name = str(df_raw.iloc[max(0, s_row-1), j]).strip().replace("\n", " ")
                if h_name.lower() in ['nan', 'none', '']: 
                    h_name = str(df_raw.iloc[max(0, s_row-2), j]).strip().replace("\n", " ")
                if h_name.lower() in ['nan', 'none', '']: h_name = f"Cột {j}"
                
                orig = h_name
                count = 1
                while h_name in [n for _, n in pcnl_headers]:
                    h_name = f"{orig} ({count})"
                    count += 1
                pcnl_headers.append((j, h_name))
        
        # Đảm bảo các cột Nhận xét tồn tại an toàn để tránh KeyError
        if loai_nx == "PC-NL":
            if pcnl_cols['nx_nlc'] == -1: 
                new_col = f"NX_NLC_{uuid.uuid4().hex[:4]}"
                df_raw.insert(len(df_raw.columns), new_col, "")
                pcnl_cols['nx_nlc'] = df_raw.columns.get_loc(new_col)
                df_raw.iat[header_r, pcnl_cols['nx_nlc']] = "NX Năng lực chung"
            if pcnl_cols['nx_nldt'] == -1: 
                new_col = f"NX_NLDT_{uuid.uuid4().hex[:4]}"
                df_raw.insert(len(df_raw.columns), new_col, "")
                pcnl_cols['nx_nldt'] = df_raw.columns.get_loc(new_col)
                df_raw.iat[header_r, pcnl_cols['nx_nldt']] = "NX Năng lực đặc thù"
            if pcnl_cols['nx_pc'] == -1: 
                new_col = f"NX_PC_{uuid.uuid4().hex[:4]}"
                df_raw.insert(len(df_raw.columns), new_col, "")
                pcnl_cols['nx_pc'] = df_raw.columns.get_loc(new_col)
                df_raw.iat[header_r, pcnl_cols['nx_pc']] = "NX Phẩm chất"
        else:
            if c_col == -1:
                new_col = f"NX_MON_{uuid.uuid4().hex[:4]}"
                df_raw.insert(len(df_raw.columns), new_col, "")
                c_col = df_raw.columns.get_loc(new_col)
                df_raw.iat[header_r, c_col] = "Nhận xét"
        ...
        for i in range(s_row, len(df_raw)):
            ...
            if ten.lower() not in ['nan', 'none', '']:
                if loai_nx == "PC-NL":
                    ...
                    if pcnl_headers:
                        for j, h_name in pcnl_headers:
                            val = str(df_raw.iloc[i, j]).strip().upper()
                            row_data[h_name] = val if val not in ['NAN', 'NONE'] else ""
                    else:
                        ...
                        row_data["Mức NLC"] = m_nlc if m_nlc not in ['NAN', 'NONE'] else ""
                        row_data["Mức NLĐT"] = m_nldt if m_nldt not in ['NAN', 'NONE'] else ""
                        row_data["Mức PC"] = m_pc if m_pc not in ['NAN', 'NONE'] else ""
                        
                    row_data["NX Năng lực chung"] = ""
                    row_data["NX Năng lực đặc thù"] = ""
                    row_data["NX Phẩm chất"] = ""
        ...
        if not data_list:
            if loai_nx == "PC-NL":
                cols = ["Họ và tên"]
                if pcnl_headers: cols.extend([h for _, h in pcnl_headers])
                else: cols.extend(["Mức NLC", "Mức NLĐT", "Mức PC"])
                cols.extend(["NX Năng lực chung", "NX Năng lực đặc thù", "NX Phẩm chất"])
        ...
        if loai_nx == "PC-NL":
            if pcnl_headers:
                for _, h_name in pcnl_headers:
                    config[h_name] = st.column_config.Column(width="small")
            else:
        ...
        if st.button("🚀 Thực hiện nhận xét Tự động"):
            ...
            for idx, row in df_view.iterrows():
                ...
                if loai_nx == "PC-NL":
                    if pcnl_headers:
                        all_vals = [str(row.get(h_name, "")).strip() for _, h_name in pcnl_headers]
                        if all(v == "" for v in all_vals): is_empty_row = True
                    else:
                        m_nlc = str(row.get("Mức NLC", "")).strip()
                        m_nldt = str(row.get("Mức NLĐT", "")).strip()
                        m_pc = str(row.get("Mức PC", "")).strip()
                        if m_nlc == "" and m_nldt == "" and m_pc == "": is_empty_row = True
                else:
                ...
                if is_empty_row:
                    ...
                    if loai_nx == "PC-NL":
                        if pcnl_headers:
                            all_evals = [str(row.get(h_name, "")).upper() for _, h_name in pcnl_headers]
                            overall = calc_overall_mdd(all_evals)
                            mdd_nlc = get_mdd(str(row.get("Mức NLC", ""))) if "Mức NLC" in row and row["Mức NLC"] else overall
                            mdd_nldt = get_mdd(str(row.get("Mức NLĐT", ""))) if "Mức NLĐT" in row and row["Mức NLĐT"] else overall
                            mdd_pc = get_mdd(str(row.get("Mức PC", ""))) if "Mức PC" in row and row["Mức PC"] else overall
                        else:
                        ...
                        if api_key:
                            prompt = f"Mã:{salt}. Đóng vai giáo viên tiểu học. Viết 3 nhận xét ngắn gọn, riêng biệt: 1. Năng lực chung (Mức {mdd_nlc}), 2. Năng lực đặc thù (Mức {mdd_nldt}), 3. Phẩm chất (Mức {mdd_pc}). PHONG CÁCH: {style_prompt}. BẮT BUỘC TRẢ VỀ THEO ĐỊNH DẠNG: [Nhận xét 1] | [Nhận xét 2] | [Nhận xét 3]. TUYỆT ĐỐI KHÔNG DÙNG TỪ 'Em', 'Về năng lực', 'Về phẩm chất', 'Nhận thấy' ở đầu câu. Nhận xét trực tiếp vào năng lực luôn. {lenh_cam_cam_xuc}"
                            try:
                        ...
                        df_raw.iat[int(real_idx), int(pcnl_cols['nx_pc'])] = nx_pc
            ...
            if st.session_state.ket_qua_nhan_xet:
                if loai_nx == "PC-NL":
                    st.warning("🚨 **LƯU Ý VỚI BẢNG PC-NL:** Bảng PC-NL chứa tới 3 cột nhận xét khác nhau. Thầy/Cô vui lòng **Tải file kết quả dự phòng** (nút bên dưới) và mở bằng Excel để sử dụng. Trên file tải về, 3 cột nhận xét đã được điền ĐÚNG VỊ TRÍ BIỂU MẪU CỦA TRƯỜNG để copy!")
                else:
